...
class XGBAdversarialRegressor(XGBModel):
    ...
    def fit(self, X, y, eval_set=None, eval_metric=None,
        ...
        prior = X
        if len(y.shape) == 1:
            posterior = np.concatenate((np.expand_dims(y,axis=-1),X),axis=-1)
        elif len(y.shape) == 2:
            posterior = np.concatenate((y,X),axis=-1)
        else: 
            raise ValueError('Numpy Arrays must be 2-D')

        posterior = DMatrix(posterior)
        ...
        self.booster = train(params, prior, posterior, distribution,
